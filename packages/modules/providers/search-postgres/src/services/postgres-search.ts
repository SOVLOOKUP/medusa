...
export class PostgresSearchService extends AbstractSearchProviderService {
  ...
  protected keywordFragments(input: {
    ...
    const { query, catalog, extensions } = input
    const plan = catalog.plan
    ...
    const wordSim = (params: unknown[]) => {
      params.push(q)
      return wordSimilarityCall(extensions, "?", textExpr)
    }
  ...
  protected async searchKeyword(input: {
    ...
    const fragments = this.keywordFragments({ query, catalog, extensions })
  ...
  protected buildFacetScope(input: {
    ...
    const { input: query, catalog, extensions, useKeyword, useVector } = input
    const params: unknown[] = []
    const conditions: string[] = []

    const matchSql = useKeyword
      ? this.keywordFragments({ query, catalog, extensions }).matchSql
      : undefined
